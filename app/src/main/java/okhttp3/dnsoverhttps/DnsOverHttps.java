...
public class DnsOverHttps implements Dns {
    ...
    private List<InetAddress> lookupHttps(String hostname) throws UnknownHostException {
        ...
        // 2026-09-19：原先这里静默回退 Dns.SYSTEM。回退本身是对的，但**外部无从得知 DoH 已经失败**，
        // 于是外层既无法熔断（每个新域名都要白等一次 HTTP 超时），日志里也看不出"安全DNS 其实一直没生效"。
        // 现改为把失败如实抛出，回退职责交给外层包装 com.github.tvbox.osc.util.L1FallbackDns：
        // 对请求层而言行为不变（照样回退系统 DNS），但失败可见、可计数、可熔断。
        // ⚠ 前提：所有 builder.dns(...) 注入点都必须挂包装类，不允许再出现裸的 builder.dns(dnsOverHttps)
        //   —— 由回归闸门断言守住。
        throw new UnknownHostException("doh failed, no result for " + hostname + " (" + failures.size() + " failures)");
    }
