...
public class JsLoader {
    ...
    private boolean loadClassLoader(String jar, String key) {
        ...
        try {
            ...
            if (!cacheDir.exists())
                cacheDir.mkdirs();
            DexClassLoader classLoader = new DexClassLoader(jar, cacheDir.getAbsolutePath(), null, JarToastBlockLoader.create(App.getInstance().getClassLoader()));
    ...
    private Class<?> loadJarInternal(String jar, String md5, String key) {
        ...
        try {
            cache.delete(); // 同 JarLoader：setReadOnly 后需先删再写
