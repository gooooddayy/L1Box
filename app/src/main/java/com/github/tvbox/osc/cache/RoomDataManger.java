...
public class RoomDataManger {
    ...
    public static void insertVodRecord(String sourceKey, VodInfo vodInfo) {
        ...
        // 去重键是 (sourceKey, vodId)：查不到就会插一条新行。这里把结果打出来，
        // 「历史记录里同一部片堆了很多条」到底是键变了还是每次都没查到，看这行即可。
        PlayTrace.diag("历史落库 " + ((record == null) ? "新建" : "更新")
                + " key=" + sourceKey + " vodId=" + vodInfo.id + " 集=" + vodInfo.playIndex);
        if (record == null) {
