...
public class PlayingControlRightDialog extends DrawerPopupView {
    ...
    private void initListener(){
        ...
        // 「切换播放器」：与控制条那颗按钮同源（同一份清单、同一套语义），先关本弹窗避免叠加
        mBinding.playerPick.setOnClickListener(view -> {
            dismiss();
            mController.mPlayerExtBtn.performClick();
        });
    ...
    private void updateSpeedUi(){
        for (int i = 0; i <mBinding.containerSpeed.getChildCount(); i++) {
            ...
            if (String.valueOf(mPlayer.getSpeed()).equals(tv.getText().toString().replace("x",""))){
                tv.setBackground(ContextCompat.getDrawable(mDetailActivity, R.drawable.bg_r_common_solid_primary));
                tv.setTextColor(ColorUtils.getColor(R.color.white));
            }else {
                tv.setBackground(ContextCompat.getDrawable(mDetailActivity, R.drawable.bg_r_common_stroke_primary));
