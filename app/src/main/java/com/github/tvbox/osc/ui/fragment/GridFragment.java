...
public class GridFragment extends BaseLazyFragment {
    ...
    /** 冷启动缓存恢复的第一页列表：非空时 initData 直接展示它，消费一次后置 null */
    private java.util.List<Movie.Video> presetVideos = null;
    ...
    /** 缓存恢复模式：presetVideos 非空时首屏直接展示上次的列表，不拉网络 */
    public static GridFragment newInstance(MovieSort.SortData sortData, java.util.List<Movie.Video> presetVideos) {
        GridFragment f = new GridFragment().setArguments(sortData);
        f.presetVideos = presetVideos;
        return f;
    }
    ...
    private void saveCurrentView(){
        if(this.mGridView == null || this.sortData == null) return;
    ...
    public boolean restoreView(){
        if(mGrids.empty() || mGridView == null || sortData == null) return false;
        this.showSuccess();
        ...
        gridAdapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            ...
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                ...
                if (video != null) {
                    ...
                    // video.tag 可能为 null（部分站点不返回），直接 .equals 会闪退
                    if(("12".indexOf(getUITag()) != -1) && ("folder".equals(video.tag) || "cover".equals(video.tag))){
                        focusedView = view;
                        changeView(video.id, "folder".equals(video.tag));
                    }
...
//                if(mGridView != null) mGridView.requestFocus();
                if (absXml != null && absXml.movie != null && absXml.movie.videoList != null && absXml.movie.videoList.size() > 0) {
                    if (page == 1) {
                        ...
                        // 首页冷启动缓存：归档第一页数据，供下次启动秒显（异步落盘，零阻塞）
                        HomeCache.putVideos(sortData != null ? sortData.id : null, absXml.movie.videoList);
                    } else {
    ...
    private void initData() {
        ...
        if (presetVideos != null && !presetVideos.isEmpty()) {
            ...
            isLoad = true;
            page = 2;
            gridAdapter.setNewData(presetVideos);
            presetVideos = null;
            return;
        }
    ...
    /**
     * 列表是否还能向上滚动（true = 不在顶部）。
     * ViewPager 不传递嵌套滚动事件，首页下拉刷新靠这个判断避免与列表滚动冲突。
     */
