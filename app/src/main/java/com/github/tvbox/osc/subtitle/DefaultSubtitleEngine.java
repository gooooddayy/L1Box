...
public class DefaultSubtitleEngine implements SubtitleEngine {
    ...
    private boolean mPendingStart = false; // 字幕已就绪但播放器未绑定时缓存 start,等 bindToMediaPlayer 触发
    ...
    public void bindToMediaPlayer(AbstractPlayer mediaPlayer) {
        ...
        if (mPendingStart && mWorkHandler != null) {
            mPendingStart = false;
    ...
    public void setSubtitlePath(final String path) {
        ...
        SubtitleLoader.loadSubtitle(path, new SubtitleLoader.Callback() {
            ...
            public void onSuccess(final SubtitleLoadSuccessResult subtitleLoadSuccessResult) {
                ...
                if (subtitlePath != null && !TextUtils.isEmpty(cacheKey)) {
            ...
            public void onError(final Exception exception) {
                ...
                try {
                    ...
                    handler.post(() -> Toast.makeText(App.getInstance(), "字幕加载失败，请确认文件可读取", Toast.LENGTH_SHORT).show());
    ...
    public void start() {
        ...
        if (mMediaPlayer == null) {
            Log.w(TAG, "MediaPlayer is not bind yet — pending start until bindToMediaPlayer.");
            mPendingStart = true;
            return;
        }
        stop();
